 ...
 * Data storage: Records are stored in memory only and will be lost on page refresh.
...
const App = () => {
  ...
  const handleAdd = (values: RecordFormValues) => {
    ...
    // Clear file name when adding new records manually
  ...
  const handleDelete = (record: ValuationRecord) => {
    ...
    // Clear file name when deleting records
    setCurrentFileName(null);
  };
  ...
  const generateCopyRef = (
    base: string,
    existing: ValuationRecord[]
  ): string => {
  ...
  const normaliseImportedRecord = (raw: unknown): ValuationRecord | null => {
    ...
    const typeOfReport =
      record.typeOfReport === "Interim Report"
        ? "Interim Report"
        : "Final Report";
  ...
  const mergeImportedRecords = (incoming: ValuationRecord[]) => {
    ...
    incoming.forEach((candidate) => {
      const existingIndex = next.findIndex(
        (record) => record.hecRefNo === candidate.hecRefNo
      );
      if (existingIndex === -1) {
        next.push(candidate);
        return;
      ...
      <header className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-800 bg-slate-900 px-6 py-4 shadow">
        <div>
          <h1 className="text-2xl font-semibold text-slate-100">
            Valuation Details Manager
          </h1>
          <p className="text-sm text-slate-400">
            Manage valuation records with inline validation and local backups.
          </p>
          {currentFileName && (
